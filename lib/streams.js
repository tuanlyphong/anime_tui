...
export async function streams(epUrl) {
  ...
  try {
    ...
    // Redirects can replace the document after goto resolves, including while
    // an API request is pending. Wait for a usable document and retry transient
    // context loss or fetch interruption, keeping the browser and cookies.
    // HTTP errors, API responses and request timeouts are not retried here.
    const evaluate = async (callback, argument) => {
      for (let attempt = 0; attempt < 3; attempt++) {
        try {
          ...
          if (attempt === 2 || !/Execution context was destroyed|Stream document not ready|TypeError: Failed to fetch/.test(error.message)) throw error;
    ...
    const challengeDetected = await evaluate(() => {
      if (!document.body) throw new Error('Stream document not ready');
    ...
    const link = await evaluate(async ({ episodeId, timeoutMs }) => {
