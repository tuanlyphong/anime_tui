...
export async function streams(epUrl) {
  ...
  try {
    ...
    // an API request is pending. Retry only this transient context-loss error,
    // keeping the browser/cookies and restarting the interrupted operation.
    const evaluate = async (callback, argument) => {
      for (let attempt = 0; attempt < 3; attempt++) {
        try {
          return await page.evaluate(callback, argument);
        } catch (error) {
          if (attempt === 2 || !/Execution context was destroyed/.test(error.message)) throw error;
