...
export async function playEpisode({ streamUrl, animeUrl, episodeUrl, label,
  ...
  const startSource = output => {
    source = streamAbyss({ jar, id: abyssId, quality, output, signal: abort.signal,
      allowQualityFallback: progressive, ...(progressive ? {} : { maxRetries: 0 }) })
  ...
  try {
    ...
    let target = previous?.state === 'unfinished' ? previous.positionSeconds : 0;
    ...
    const args = [...playerArgs, ...(streaming ? progressivePlayerArgs : []),
      // keep-open suppresses end-file at EOF, preventing completion/auto-next.
      '--keep-open=no', '--idle=yes', '--pause=yes', `--input-ipc-server=${socket}`,
    ...
    playerProcess.exit.then(({ code, signal, error }) => finish(error || code || signal || !client ?
      { reason: 'error', error: error || new Error(signal ? `Player terminated by ${signal}` : !client ? 'Player exited before IPC tracking was established' : `Player exited with status ${code}`) } : { reason: 'quit' }));
    ...
    client.on('disconnect', error => {
      ...
      Promise.race([playerProcess.exit, delay(100).then(() => ({ error }))])
        .then(({ code, signal, error: exitError }) => finish(exitError || code || signal ?
          { reason: 'error', error: exitError || (signal ? new Error(`Player terminated by ${signal}`) : error) } : { reason: 'quit' }));
    });
    ...
    // End timestamps may exceed duration by a fraction of a frame. Replay the
    // final second instead of waiting for an unreachable cache position; only
    // actual EOF below can mark completion and trigger auto-next.
    if (validTime(duration) && duration > 0) target = Math.min(target, Math.max(0, duration - 1));
    if (target > 0) {
      if (streaming) {
